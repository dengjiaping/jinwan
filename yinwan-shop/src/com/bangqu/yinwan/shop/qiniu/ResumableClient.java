...
public class ResumableClient extends Client {
	...
	public ICancel[] putblock(final InputStreamAt input, final PutExtra extra, final PutRet putRet,
			final long offset, final JSONObjectRet callback) {
		...
		JSONObjectRet ret = new JSONObjectRet() {
			...
			public void putNext() {
				wrote = putRet.offset;
				int remainLength = Math.min((int) (input.length() - offset - putRet.offset),
						CHUNK_SIZE);
				try {
				...
				canceler[0] = bput(putRet.host, input, putRet.ctx, offset, putRet.offset,
						remainLength, this);
			}
			...
			public void onSuccess(JSONObject obj) {
				...
				if (extra.notify != null)
					extra.notify.onSuccessUpload(extra);
				wrote += writing;
	...
	public ICancel bput(String host, InputStreamAt input, String ctx, long blockOffset,
			long offset, int writeLength, CallRet ret) {
	...
	public ICancel mkfile(String key, long fsize, String mimeType, Map<String, String> params,
			String ctxs, CallRet ret) {
