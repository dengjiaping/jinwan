...
public class IO {
	...
	private static Client defaultClient() {
		if (mClient != null && System.currentTimeMillis() - mClientUseTime > 3 * 60 * 1000) { // 1
																							// minute
			mClient.close();
			mClient = null;
		}
	 ...
	 * @param ret	  回调函数
	 */
	public void put(String key, InputStreamAt isa, PutExtras extra, JSONObjectRet ret) {
		MultipartEntity m = new MultipartEntity();
		if (key != null)
			m.addField("key", key);
		if (extra.checkCrc == PutExtras.AUTO_CRC32) {
			try {
		...
		if (extra.checkCrc != PutExtras.UNUSE_CRC32)
	 ...
	 *            结果回调函数
	 */
	public void putFile(Context mContext, String key, Uri uri, PutExtras extra,
	...
	public void putFile(String key, File file, PutExtras extra, JSONObjectRet callback) {
		putAndClose(key, InputStreamAt.fromFile(file), extra, callback);
	}

	public void putAndClose(final String key, final InputStreamAt input, final PutExtras extra,
			final JSONObjectRet ret) {
	...
	public static void put(String uptoken, String key, InputStreamAt input, PutExtras extra,
	...
	public static void putFile(Context mContext, String uptoken, String key, Uri uri,
			PutExtras extra, JSONObjectRet callback) {
		new IO(defaultClient(), uptoken).putFile(mContext, key, uri, extra, callback);
	}

	public static void putFile(String uptoken, String key, File file, PutExtras extra,
			JSONObjectRet callback) {
